    ...
    public:
                                ...
                                : settings(settingsIn), currentTolerance(currentToleranceIn)
        {
            ...
            configureIntegerEditor(adjustMethodEditor,
                                   ...
                                   1, 2);
            configureIntegerEditor(dragReturnDelayEditor,
                                   settings.getPointerControlDragReturnDelayMs(),
                                   0, 5000);
...
MainComponent::MainComponent()
{
    ...
    audioEngine.restoreAudioDeviceStateFromSettings(settings);
    ...
    audioEngine.addDeviceManagerChangeListener(this);
    ...
    addAndMakeVisible(presetLoadingOverlay);
    presetLoadingOverlay.setVisible(false);
    presetLoadingOverlay.setJustificationType(juce::Justification::centred);
    presetLoadingOverlay.setColour(juce::Label::backgroundColourId, juce::Colour(0xE01D2230));
    presetLoadingOverlay.setColour(juce::Label::textColourId, juce::Colours::white);
    presetLoadingOverlay.setFont(juce::Font(juce::FontOptions(20.0f, juce::Font::bold)));
    presetLoadingOverlay.setText("Loading preset...", juce::dontSendNotification);
    ...
    midiEngine.restoreEnabledDevicesFromSettings(settings);
    ...
    midiEngine.onMidiMessageReceived = [this](const MidiEngine::IncomingMidiMessage& incoming)
    {
        if (incoming.message.isController())
        {
            ...
            if (cc == pointerXcc || cc == pointerYcc || cc == pointerToleranceCc)
...
MainComponent::~MainComponent()
{
    ...
    audioEngine.removeDeviceManagerChangeListener(this);
    midiEngine.removeChangeListener(this);
    standaloneTempoSupport.shutdown(audioEngine.getDeviceManager());
    audioEngine.saveAudioDeviceStateToSettings(settings);
...
void MainComponent::timerCallback()
{
    ...
    pointerControl.releaseDragIfIdle((double) settings.getPointerControlDragReturnDelayMs());
...
void MainComponent::resized()
{
    ...
    presetLoadingOverlay.setBounds(area);
    ...
    if (presetLoadingOverlay.isVisible())
        presetLoadingOverlay.toFront(false);
...
void MainComponent::menuItemSelected(int itemId, int)
{
    ...
    if (itemId >= 1000 && itemId < 1300)
    {
        ...
        if (juce::isPositiveAndBelow(deviceIndex, devices.size()))
        {
            ...
            midiEngine.saveEnabledDevicesToSettings(settings);
    ...
    switch (itemId)
    {
        ...
        case 201:
        {
            audioEngine.showAudioSettingsDialog();
            break;
        }
        case 202:
            toggleRoutingView();
...
bool MainComponent::confirmOverwritePresetFile(const juce::File& file) const
{
    ...
    return juce::AlertWindow::showOkCancelBox(
        ...
        "The preset file below already exists:\n\n"
            + file.getFullPathName()
            + "\n\nSaving now will permanently overwrite it.\n\n"
...
void MainComponent::savePresetAs()
{
    ...
    if (!confirmOverwritePresetFile(file))
        return;
...
bool MainComponent::loadPresetFromFile(const juce::File& file)
{
    ...
    setPresetLoadingOverlayVisible(true);
    ...
    setPresetLoadingOverlayVisible(false);
    return true;
}
...
void MainComponent::setPresetLoadingOverlayVisible(bool shouldShow)
{
    presetLoadingOverlay.setVisible(shouldShow);
...
{
    midiEngine.refreshDevicesFromSettings(settings);
    midiEngine.saveEnabledDevicesToSettings(settings);
