...
void MidiEngine::restoreEnabledDevicesFromSettings(const AppSettings& settings)
{
    ...
    if (enabledDeviceIdentifiers.isEmpty())
    {
        ...
        if (legacySavedMidiDevice.isNotEmpty())
            openDevice(legacySavedMidiDevice);
    }
}

void MidiEngine::saveEnabledDevicesToSettings(AppSettings& settings) const
{
    auto openNames = getOpenDeviceNames();
    ...
    settings.setMidiDeviceName(openNames[0]);
    settings.setEnabledMidiDeviceIdentifiers(getOpenDeviceIdentifiers());
}

void MidiEngine::refreshDevicesFromSettings(const AppSettings& settings)
{
    auto enabledDeviceIdentifiers = settings.getEnabledMidiDeviceIdentifiers();

    if (enabledDeviceIdentifiers.isEmpty())
        enabledDeviceIdentifiers = getOpenDeviceIdentifiers();

    refreshDevices(enabledDeviceIdentifiers);
}
