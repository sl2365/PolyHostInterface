...
namespace
{
    ...
    std::vector<InstructionTopic> buildTopics()
    {
        ...
        topics.push_back({ "Presets", makeBody("Presets",
            "PHI presets save the current tab layout, loaded plugins, plugin states, MIDI assignments, routing-related tab state, selected external pointer maps, Macro mappings, paused/enabled mapping states, and Seqwencer A-H target assignments.\n\n"
        ...
        topics.push_back({ "MIDI Keyboard", makeBody("MIDI Keyboard",
            "Options > Keyboard > Show displays or hides PHI's MIDI keyboard at the bottom of the window. It remains at the bottom in the normal plug-in view, Routing View and Macro Mappings. Routing rows and Macro Mapping rows stop above it rather than scrolling beneath it.\n\n"
        ...
        topics.push_back({ "Macro Mapping", makeBody("Macro Mapping",
            ...
            "When Seqwencer is loaded, the same view adds a Targets column. Tick any lanes from A-H to let those sequencers control the parameter. In Parallel the lanes are independent; in SERIAL each A/B, C/D, E/F or G/H pair mirrors one shared 64-step target while its second lane's saved Parallel assignment is preserved. Seqwencer's PHI > TARGETS button opens this integrated Macro Mappings view directly. The Targets column is completely removed when Seqwencer is not loaded.\n\n"
            "Removing Seqwencer does not delete these mappings or their saved A-H target bits. PHI keeps them with the preset, hides the Targets column, and simply receives no Seqwencer modulation. Loading Seqwencer again reveals and resumes the saved assignments. Use the Macro-column X only when you intend to delete the complete Macro mapping and all of its Seqwencer assignments.\n\n"
        ...
        topics.push_back({ "Seqwencer Integration", makeBody("Seqwencer Integration",
            ...
            "FX AND TARGETS\n\n"
            "Click the GATE or PHI body in Seqwencer's left FX rail to show that section's controls. The small LED inside each FX button independently enables or disables it. PHI appears only when Seqwencer detects PHI. Open PHI controls and click TARGETS to open PHI's integrated Macro Mappings view. The A/B, C/D, E/F and G/H buttons beside TARGETS choose which PHI sequencer pair is displayed; all four pairs continue running.\n\n"
            "Tick any lanes from A-H beside a hosted parameter. PHI creates the next free Macro automatically if the parameter was not already mapped. Unticking a lane stops it controlling the destination but keeps its Macro. Unticking Mapped pauses all Macro and Seqwencer control without deleting the number. The Macro-column X deletes the full mapping.\n\n"
            "PARALLEL AND SERIAL\n\n"
            "Each A/B, C/D, E/F and G/H pair has its own Rate, range, Direction, lane enable states, Attack, Release, Bipolar and step settings. Parallel runs the two 32-step lanes in a pair independently. SERIAL plays the pair's first lane followed by its second lane as one 64-step sequence. The first lane is the shared SERIAL destination set; the second target display mirrors it while the saved second-lane Parallel assignments remain intact. Both grids continue playing.\n\n"
            "GATE CONTROLS\n\n"
